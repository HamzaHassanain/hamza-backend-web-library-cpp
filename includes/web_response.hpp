...
namespace hamza_web
{
    ...
    public:
        ...
        {
            response.add_header(hamza_http::headers::CONTENT_TYPE, "application/json");
            response.set_body(json_data);
        }

        void html(const std::string &html_data)
        {
            response.add_header(hamza_http::headers::CONTENT_TYPE, "text/html");
            response.set_body(html_data);
        }

        void text(const std::string &text_data)
        {
            response.add_header(hamza_http::headers::CONTENT_TYPE, "text/plain");
        ...
        {
            response.add_header(hamza_http::headers::CONTENT_TYPE, content_type);
        }
