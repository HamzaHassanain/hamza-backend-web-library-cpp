...
namespace hamza_web
{
    ...
    public:
        ...
        {
            return request.get_header(hamza_http::headers::CONTENT_TYPE);
        }

        std::vector<std::string> get_cookies() const
        {
            return request.get_header(hamza_http::headers::COOKIE);
        }

        std::vector<std::string> get_authorization() const
        {
            return request.get_header(hamza_http::headers::AUTHORIZATION);
        }
    };
}